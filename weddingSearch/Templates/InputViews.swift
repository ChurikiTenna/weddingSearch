...
class SwitchViewAndTtl: UIView {
    
    var ttlLbl: UILabel!
    var switchView: SwitchView!
    
    init(_ f: inout CGRect, ttl: String, options: [String], delegate: SwitchViewDelegate?, selectedIdx: Int?, to view: UIView) {
        super.init(frame: f)
        view.addSubview(self)
        
        ttlLbl = UILabel.grayTtl(CGRect(x: 10, y: 0, w: w, h: 20), ttl: ttl, to: self)
        switchView = SwitchView(CGRect(y: ttlLbl.maxY+10, w: f.width, h: 40), to: self)
        if let selectedIdx = selectedIdx {
            switchView.selectedIdx = selectedIdx
        }
        switchView.initUI(roundRatio: 0.5, options: options, delegate: delegate)
        
        frame.size.height = switchView.maxY
        f.origin.y = maxY+10
    }
...
protocol TextInputViewDelegate: AnyObject {
    func sendBtnTapped(with text: String)
}
class TextInputView: UIView {
    ...
    init(delegate: TextInputViewDelegate, to view: UIView) {
        self.delegate = delegate
        super.init(frame: CGRect(y: view.h-120, h: view.h))
        backgroundColor = .white
        view.addSubview(self)
        
        topBar()
        // 入力フィールド
        textView = UITextView(CGRect(x: 20, y: 10, w: view.w-70, h: 80), textSize: 16, to: self)
        // 送信
        let sendDMBtn = ImageBtn(CGPoint(x: view.w-45, y: (100-30)/2),
                                 image: .paperplane, width: 30, theme: .clearBlack, to: self)
        sendDMBtn.addTarget(self, action: #selector(sendBtnTapped), for: .touchUpInside)
    ...
    @objc func keyboardWillShow(_ notification: Notification) {
        ...
        // bottom of textField
        let bottomTextField = frame.origin.y + textView.frame.maxY+10
        // top of keyboard
        let topKeyboard = superview.h - keyboardFrame.size.height
        // 重なり
        let distance = bottomTextField - topKeyboard
        
        if distance >= 0 {
            superview.frame.origin.y = -distance
        }
    }
    ...
    @objc func sendBtnTapped() {
        delegate?.sendBtnTapped(with: textView.text)
    }
}
